...
%%
% Plot exact and approximate distributions against data
figure;
ksdensity([T1 T2]); hold on;
t = linspace(0,5e4,200);
fe = zeros(size(t));
fa = zeros(size(t));
for i = 1:length(t)
   fe(i) = exp(loglikelihood(t(i),[l1,l2],x01,[p1,p2],'exact'));
   fa(i) = exp(loglikelihood(t(i),[l1,l2],x01,[p1,p2],'approx'));
end
plot(t,fe); hold on;
plot(t,fa);
